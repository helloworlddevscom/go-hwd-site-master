...
class TVO_Block {
	...
	public static function can_use_blocks() {
		return is_plugin_active( 'thrive-visual-editor/thrive-visual-editor.php' ) && function_exists( 'register_block_type' );
	}
	...
	public static function register_block() {
		...
		wp_register_script( 'tvo-capture-block-editor', TVO_URL . 'blocks/build/capture.js', $capture_asset_file['dependencies'], $capture_asset_file['version'], false );

		register_block_type(
			'thrive/' . self::NAME_CAPTURE,
			array(
				'render_callback' => 'TVO_Block::render_block',
				'editor_script'   => 'tvo-capture-block-editor',
				'editor_style'    => 'tvo-capture-block-editor',
			)
		);

		wp_register_script( 'tvo-display-block-editor', TVO_URL . 'blocks/build/display.js', $display_asset_file['dependencies'], $display_asset_file['version'], false );

		register_block_type(
			'thrive/' . self::NAME_DISPLAY,
			array(
				'render_callback' => 'TVO_Block::render_block',
				'editor_script'   => 'tvo-display-block-editor',
	...
	public static function get_content( $block_id ) {
		...
		if ( TVO_Block::can_use_blocks() ) {
			...
			$content = apply_filters( 'tvp_block_template', $content );
