...
class TCB_Course_Rest_Controller {
	...
	private function get_course_data( $id, $include_structure = false ) {
		...
		foreach ( $all_content as $key => $post ) {
			...
			$all_content[ $key ]->tva_course_status                    = $courseItem->is_completed() ? __( 'Completed', TVA_Const::T ) : __( 'Not Completed', TVA_Const::T );
			$all_content[ $key ]->tva_course_type                      = $courseItem->get_type();
			$all_content[ $key ]->tva_course_type_svg                  = \TVA\Architect\Course\tcb_course_shortcode()->get_type_icon( $courseItem );
			$all_content[ $key ]->tva_course_children_count_with_label = \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $courseItem );
			$all_content[ $key ]->tva_course_restriction_label         = \TVA\Architect\Course\tcb_course_shortcode()->get_course_label( $course );
		}
