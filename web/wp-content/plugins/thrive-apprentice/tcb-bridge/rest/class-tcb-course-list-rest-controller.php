...
class TCB_Course_List_Rest_Controller {
	...
	public function get_query_data( $request ) {
		...
		array_unshift( $labels, array(
			'ID'    => 'all',
			'title' => esc_attr( __( 'All restricted content levels', \TVA_Const::T ) ),
		), array(
			'ID'   => - 1000,
			'name' => esc_attr( __( 'No restriction - free to all visitors', \TVA_Const::T ) ),
		) );

		array_unshift( $levels, array(
			'ID'   => 'all',
			'name' => esc_attr( __( 'All difficulty levels', \TVA_Const::T ) ),
		) );
		...
		return new WP_REST_Response( array(
			...
			'progress' => array(
				array(
					'ID'    => 'all',
					'title' => esc_attr( __( 'All course progress', \TVA_Const::T ) ),
				),
				array(
					'ID'    => TVA_Const::TVA_COURSE_PROGRESS_NO_ACCESS,
					'title' => esc_attr( __( 'No access', \TVA_Const::T ) ),
				),
				array(
					'ID'    => 'progress-has-access',
					'title' => esc_attr( __( 'Has access - all progress', \TVA_Const::T ) ),
				),
				array(
					'ID'    => TVA_Const::TVA_COURSE_PROGRESS_NOT_STARTED,
					'title' => esc_attr( __( 'Not started', \TVA_Const::T ) ),
				),
				array(
					'ID'    => TVA_Const::TVA_COURSE_PROGRESS_IN_PROGRESS,
					'title' => esc_attr( __( 'In Progress', \TVA_Const::T ) ),
				),
				array(
					'ID'    => TVA_Const::TVA_COURSE_PROGRESS_COMPLETED,
					'title' => esc_attr( __( 'Completed', \TVA_Const::T ) ),
				),
			),
		), 200 );
	}
