...
function tva_ajax_landing_page_cloud() {
	...
	if ( empty( $_POST['task'] ) ) {
		$error = __( 'Invalid request', TVA_Const::T );
	}
		...
		try {
			switch ( $_POST['task'] ) {
				case 'download':
					...
					if ( empty( $template ) ) {
						throw new Exception( __( 'Invalid template', TVA_Const::T ) );
					}
...
add_action( 'init', 'tva_load_conditional_display_classes', 11 );
...
function tva_load_tcb_classes() {
	...
	require_once __DIR__ . '/classes/dynamic-video/class-main.php';
	TVA\Architect\DynamicVideo\Main::instance();

	require_once __DIR__ . '/classes/resources/class-main.php';
	require_once __DIR__ . '/classes/course/class-main.php';
	require_once __DIR__ . '/classes/course-list/class-main.php';
	require_once __DIR__ . '/classes/course-list/class-dropdown.php';
	require_once __DIR__ . '/classes/dynamic-actions/class-main.php';
	require_once __DIR__ . '/classes/visual-builder/class-main.php';
	require_once __DIR__ . '/classes/class-utils.php';
}

/**
 * Loads the class used for Conditional Display in TAR
 */
function tva_load_conditional_display_classes() {
	if ( class_exists( '\TCB\ConditionalDisplay\Main', false ) ) {
		require_once __DIR__ . '/classes/conditional-display/class-main.php';

		\TVA\Architect\ConditionalDisplay\Main::init();
	}
}
	...
	if ( get_post_type() === TVA_Access_Restriction::POST_TYPE ) {
		...
		$data['Thrive Apprentice'] = array(
			'links'     => array(
				...
				array(
					array(
						'name' => __( 'Login & registration page', TVA_Const::T ),
						'show' => true,
						'id'   => 'login',
						'url'  => $tva_shortcodes->dynamic_link( array( 'id' => 'login' ) ),
					),
				),
			),
			'options'   => array(
				'login' => array(
					'label'   => __( 'Default state', TVA_Const::T ),
					'type'    => 'select',
					'options' => array(
						'login'    => __( 'Login', TVA_Const::T ),
						'register' => __( 'Register', TVA_Const::T ),
					),
				),
			),
