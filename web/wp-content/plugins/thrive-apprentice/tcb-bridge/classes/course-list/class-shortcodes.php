...
class Shortcodes {
	 ...
	 * @return string
	 */
	public function course_item( $attr = array(), $content = '' ) {
		$attributes = array(
		...
		if ( ! empty( $attr['tcb-events'] ) ) {
			$attributes['data-tcb-events'] = str_replace( array( '|{|', '|}|' ), array( '[', ']' ), $attr['tcb-events'] );
		}

		$classes = array( 'tva-course-list-item', 'thrv_wrapper', 'thrive-animated-item' );
	...
	public function module_number_with_label( $attr = array(), $content = '' ) {
		$label = \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $this->get_active_course()->get_published_modules() );
	...
	public function chapter_number_with_label( $attr = array(), $content = '' ) {
		$label = \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $this->get_active_course()->get_published_chapters() );
	...
	public function lesson_number_with_label( $attr = array(), $content = '' ) {
		$label = \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $this->get_active_course()->get_published_lessons() );
