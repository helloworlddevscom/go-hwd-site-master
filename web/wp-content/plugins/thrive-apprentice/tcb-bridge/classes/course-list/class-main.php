...
class Main {
	...
	private function __construct() {
		...
		$this->class_shortcodes = require_once __DIR__ . '/class-shortcodes.php';
	...
	public function localize_dynamic_fields( $courses = array() ) {
		...
		foreach ( $courses as $index => $course ) {
			...
			$localization[] = array(
				...
				'module_number_with_label'  => \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $course->get_published_modules() ),
				'chapter_number_with_label' => \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $course->get_published_chapters() ),
				'lesson_number_with_label'  => \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_children_count_with_label( $course->get_published_lessons() ),
	...
	public function get_progress_percentage( $course ) {
		...
		$completed = tva_count_completed_lessons( Main::$is_editor_page ? $course->get_all_lessons() : $course->get_published_lessons() );

		return \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->get_progress_by_type( 'course', $completed, $total );
	}
	...
	public function get_type_icon( $course ) {
		...
		$icon = tva_get_file_contents( $file, [ 'title' => \TVA_Dynamic_Labels::get( 'course_labels' )[ $course->type ]['title'] ] );
	...
	private function default_args() {
		return array(
			'query'               => '',
			'ct'                  => 'course_list-123',
			'ct-name'             => esc_attr__( 'Default Template', \TVA_Const::T ),
			'type'                => 'grid',
			'columns-d'           => 3,
			'columns-t'           => 2,
			'columns-m'           => 1,
			...
			'posts_per_page'      => 6,
			'no_posts_text'       => esc_attr__( 'There are no courses to display.', \TVA_Const::T ),
