...
class Main {
	...
	public function render( $attr = array(), $content = '' ) {
		...
		if ( ! self::$is_editor_page && ! $this->course->is_published() && false === \TVA_Product::has_access() ) {
			//For unpublished courses we return nothing if it is not in the editor page or preview the module
			return '';
		}
	...
	public function allow_smart_autocollapse() {
		return ! empty( self::$course_attr['autocollapse'] ) && (int) self::$course_attr['autocollapse'] === 1 && ! empty( \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_object() );
	}
	...
	public function should_be_expanded( $post_id ) {
		...
		if ( $post_id === \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_object()->ID ) {
			return true;
		}

		$ancestors = \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_object_ancestors();
