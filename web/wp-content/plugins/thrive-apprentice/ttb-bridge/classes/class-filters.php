...
class Filters {
		...
		if ( Main::uses_builder_templates() ) {
			if ( tva_get_settings_manager()->is_index_page( get_the_ID() ) ) {
				...
				$meta = [
					...
					THRIVE_SECONDARY_TEMPLATE => is_tax( \TVA_Const::COURSE_TAXONOMY ) || in_array( get_post_type(), [ \TVA_Const::LESSON_POST_TYPE, \TVA_Const::MODULE_POST_TYPE ] ) ? \TVA_Const::NO_ACCESS : \TVA_Const::NO_ACCESS_POST,
	...
	public static function breadcrumb_root_items( $items, $index, $in_editor = false ) {
		...
		if ( is_singular( [ \TVA_Const::MODULE_POST_TYPE, \TVA_Const::LESSON_POST_TYPE, \TVA_Const::NO_ACCESS ] ) ) {
	...
	public static function hide_post_options( $element_config ) {
		if ( Check::course_item() || Check::course_overview() ) {
	...
	public static function theme_default_templates( $templates = [], $args = [], $template_meta = [] ) {
		...
		if ( $template_meta['secondary_template'] === \TVA_Const::COURSE_TAXONOMY && Check::course_overview() ) {
			$object = get_queried_object();
