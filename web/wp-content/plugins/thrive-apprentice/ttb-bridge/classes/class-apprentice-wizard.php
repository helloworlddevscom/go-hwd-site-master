...
use Thrive\Theme\Integrations\WooCommerce\Wizard;
...
class Apprentice_Wizard extends \Thrive_Wizard {
	...
	public static function get_data( $skin_id ) {
		 ...
		 */
		$lesson_templates = array_filter( static::get_templates( 'lesson' ), static function ( $template ) {
	...
	public function init_frontend() {
		...
		if ( $this->is_template_preview() ) {
			add_action( 'theme_template_before_render', function ( $template ) {
				...
				if ( $step === 'module' || strpos( $step, 'lesson' ) !== false ) {
