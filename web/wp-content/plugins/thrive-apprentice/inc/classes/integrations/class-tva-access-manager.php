...
class TVA_Access_Manager {
	...
	public function get_product() {
		...
		if ( false === $this->product instanceof \TVA\Product ) {
			$product = \TVA\Product::get_from_set();
	 ...
	 *
	 * @param \TVA\Product|WP_Term $product
	 *
	 * @return $this
	 */
	public function set_product( $product ) {
		...
		if ( $product instanceof WP_Term ) {
			$this->product = new \TVA\Product( $product );
		} else if ( $product instanceof \TVA\Product ) {
			$this->product = $product;
		}
	...
	public function get_login_redirect_url() {
		...
		if ( false === $this->has_access() && $this->get_product() instanceof \TVA\Product && \TVD\Content_Sets\Utils::is_context_supported() ) {
	...
	public function restrict_theme_section_content( $section_html, $theme_section ) {
		...
		if ( ! is_editor_page_raw( true ) && \Thrive_Theme::is_active() && \Thrive\Theme\Integrations\WooCommerce\Helpers::is_woo_template() && $theme_section->type() === 'content' && $this->should_restrict_content() ) {
	...
	public function restrict_content( $content ) {
		...
		if ( get_post_type() === 'tcb_lightbox' ) {
			 ...
			 */
			$product = null;
			$sets    = \TVD\Content_Sets\Set::get_for_object( get_post(), get_the_ID() );
		...
		} else if ( ! $this->has_access() && $product instanceof \TVA\Product ) {
	...
	public function has_access_to_object( $post_or_term ) {
		...
		list( $content_type, $id ) = \TVD\Content_Sets\Utils::get_post_or_term_parts( $post_or_term );
		...
		if ( TVA_Product::has_access() || $this->has_freemium_access( $post_or_term ) ) {
			...
			$sets = empty( $id ) ? \TVD\Content_Sets\Set::get_for_non_object() : \TVD\Content_Sets\Set::get_for_object( $post_or_term, $id );
	...
	public function is_object_locked( $post_or_term ) {
		...
		if ( is_user_logged_in() && ! TVA_Product::has_access() ) {
			...
			if ( ! empty( $course_id ) ) {
				...
				$products = \TVA\Product::get_from_set( \TVD\Content_Sets\Set::get_for_object( $post_or_term, $id ), $options, $post_or_term );

				if ( $products instanceof \TVA\Product ) {

					$campaign = $products->get_drip_campaign_for_course( $course_id );
					if ( $campaign instanceof \TVA\Drip\Campaign && ! $campaign->should_unlock( $products->get_id(), $id ) ) {
						$is_locked = true;
					}
					...
					foreach ( $products as $product ) {
						...
						if ( empty( $campaign ) || ( $campaign instanceof \TVA\Drip\Campaign && $campaign->should_unlock( $product->get_id(), $id ) ) ) {
							$is_locked = false;
							break;
						}
					}
