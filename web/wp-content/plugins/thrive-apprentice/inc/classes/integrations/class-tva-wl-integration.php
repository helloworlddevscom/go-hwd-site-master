...
class TVA_WL_Integration extends TVA_Integration {
	...
	public function get_users_with_level_query_part( $levels = array() ) {
		...
		if ( empty( $WishListMemberInstance ) || empty( $WishListMemberInstance->Tables ) || empty( $WishListMemberInstance->Tables->userlevels ) ) {
			return '';
		}
		...
		return $wpdb->prepare( "ID IN (SELECT user_id FROM {$WishListMemberInstance->Tables->userlevels} WHERE level_id IN (" . implode( ',', $params ) . "))", $levels );
	}
}
