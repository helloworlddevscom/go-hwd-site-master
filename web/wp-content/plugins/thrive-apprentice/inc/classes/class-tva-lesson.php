...
class TVA_Lesson extends TVA_Post {
	 ...
	 * @return bool
	 */
	public function is_last_visible_lesson() {

		return empty( $this->get_next_visible_lesson() );
	}
	 ...
	 * @return bool
	 */
	public function is_first_visible_lesson() {
		$lessons = $this->get_course_v2()->get_ordered_visible_lessons();

		if ( ! empty( $lessons ) ) {
			return $this === reset( $lessons );
		}
	...
	public function can_be_marked_as_completed() {
		...
		$marked_as_completed_conditions = get_post_meta( $this->ID, TVA_Const::TVA_META_NAME_CAN_MARK_COMPLETE, true );
		 ...
		 * @param int     $lesson_id
		 */
		return apply_filters( 'tva_can_be_marked_as_completed', true, $marked_as_completed_conditions, $this->ID );
	}
}
