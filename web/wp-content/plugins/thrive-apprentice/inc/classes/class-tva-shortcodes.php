	...
	public function dynamic_link( $attr ) {
		...
		switch ( $attr['id'] ) {
			case 'login':
				...
				$result = add_query_arg( 'ret', rawurlencode( is_tax( TVA_Const::COURSE_TAXONOMY ) ? tva_course()->get_link() : get_permalink() ), $result );
				if ( ! empty( $attr['opt'] ) ) {
	...
	public function post_progress( $attrs, $content = '', $tag = '' ) {
		...
		if ( ! empty( $attrs['context'] ) ) {
			if ( $attrs['context'] === 'course-list' ) {
				...
				$course_list_course = \TVA\Architect\Course_List\tcb_course_list_shortcode()->get_shortcodes_class()->get_active_course();
				...
				if ( empty( $course_list_course ) ) {
					...
					$lessons = $current_course->get_published_lessons();
				}
				...
				if ( empty( Course\Main::$parent_item ) ) {
					...
					if ( $attrs['parent'] === 'course' ) {
						...
						if ( Course\Main::$parent_item instanceof TVA_Module ) {
							$lessons = Course\Main::$parent_item->get_lessons( array( 'post_status' => 'publish' ) );
						} else if ( Course\Main::$parent_item instanceof TVA_Chapter ) {
							$lessons = Course\Main::$parent_item->get_parent()->get_lessons( array( 'post_status' => 'publish' ) );
						} else if ( Course\Main::$parent_item instanceof TVA_Lesson ) {
							$lessons = Course\Main::$parent_item->get_parent_by_type( 'tva_module' )->get_lessons( array( 'post_status' => 'publish' ) );
						}
						...
						if ( Course\Main::$parent_item instanceof TVA_Chapter ) {
							$lessons = Course\Main::$parent_item->get_lessons( array( 'post_status' => 'publish' ) );
						} else if ( Course\Main::$parent_item instanceof TVA_Lesson && Course\Main::$parent_item->get_parent() instanceof TVA_Chapter ) {
							$lessons = Course\Main::$parent_item->get_parent()->get_lessons( array( 'post_status' => 'publish' ) );
						}
					}
				}
			...
			if ( $attrs['parent'] === 'course' ) {
				...
				if ( $parent ) {
					$lessons = TVA_Post::factory( $parent )->get_lessons( array( 'post_status' => 'publish' ) );
				}
			}
		}
		...
		if ( ! empty( $learned_lessons ) ) {
			foreach ( $lessons as $lesson ) {
				if ( is_array( $learned_lessons ) && in_array( $lesson->ID, array_keys( $learned_lessons ) ) ) {
					$completed ++;
				}
			}
		}
