...
class TVA_User {
	 ...
	 * @var array
	 */
	public static $bought_products = array();
	...
	public function has_bought( $product_id ) {
		...
		foreach ( $this->get_orders_by_status( TVA_Const::STATUS_COMPLETED ) as $order ) {
			if ( true === $order instanceof TVA_Order
				 && true === ( $order_item = $order->get_order_item_by_product_id_and_status( $product_id, 1 ) ) instanceof TVA_Order_Item
			) {
				return $order;
			}
		}
	...
	/**
	 * Return the products ids that were bought by the user
	 *
	 * @return mixed
	 */
	public function get_bought_products() {
		if ( isset( static::$bought_products[ $this->get_ID() ] ) ) {
			return static::$bought_products[ $this->get_ID() ];
		}
		static::$bought_products[ $this->get_ID() ] = [];
		$sendowl_is_connected                       = TVA_SendOwl::is_connected();

		foreach ( \TVA\Product::get_items() as $product ) {
			$order = $this->has_bought( $product->ID );

			if ( $order instanceof TVA_Order ) {
				static::$bought_products[ $this->get_ID() ][] = $product->ID;
			} else if ( $sendowl_is_connected ) {
				$tva_term = new TVA_Term_Model( $product->get_term() );

				if ( $tva_term->is_protected_by_sendowl() ) {
					foreach ( $tva_term->get_all_sendowl_protection_ids() as $sendowl_protection_id ) {
						$order = $this->has_bought( $sendowl_protection_id );
