...
class TVA_Post implements JsonSerializable {
	...
	public function is_in_progress() {
		 ...
		 */
		$active_object = \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_object();
	...
	public function get_structure() {
		...
		if ( null === $this->_structure ) {
			$this->_structure = $this->init_structure();
		}
	...
	public function is_content_visible() {
		...
		if ( ! is_bool( $this->_is_content_visible ) ) {
			...
			$product = \TVA\Product::get_from_set( \TVD\Content_Sets\Set::get_for_object( $course->get_wp_term(), $course->get_id() ), array(), $course->get_wp_term() );

			if ( $product instanceof \TVA\Product ) {
				$campaign = $product->get_drip_campaign_for_course( $this->get_course_v2()->get_id() );

				if ( $campaign instanceof \TVA\Drip\Campaign && ! $campaign->should_unlock( $product->get_id(), $this->ID ) ) {
