...
class TVA_Module extends TVA_Post {
	...
	public function get_published_lessons() {
		...
		if ( false === isset( $this->_data['published_lessons'] ) ) {
			$posts   = TVA_Manager::get_all_module_lessons( $this->get_the_post(), array( 'post_status' => 'publish' ) );
			$lessons = array();
			foreach ( $posts as $post ) {
	...
	public function get_lessons( $filters = array() ) {
		...
		$lessons = TVA_Manager::get_all_module_lessons( $this->_post, $filters );
	...
	public function init_structure() {
		...
		if ( ! empty( $module_lessons ) ) {
			...
			foreach ( $module_lessons as $post_lesson ) {
				...
				$tva_lesson->set_course_v2( $this->course );
		...
		if ( ! empty( $module_chapters ) ) {
			foreach ( $module_chapters as $post_chapter ) {
				...
				$chapter->set_course_v2( $this->course );
		...
		if ( empty( $this->_structure ) ) {
			$this->_structure = array(); //the module doesn't have any child item(it's empty)
		}
	...
	public function save() {
		...
		if ( $this->get_course_v2() && false === $this->get_course_v2()->editable_module() && ! \TVA\TTB\Main::uses_builder_templates() ) {
