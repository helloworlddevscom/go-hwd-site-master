...
if ( ! class_exists( 'Thrive_Wizard_Rest' ) ) {
	require ARCHITECT_INTEGRATION_PATH . '/classes/endpoints/class-thrive-wizard-rest.php';
}
...
class TVA_Wizard_Controller extends Thrive_Wizard_Rest {
	...
	public static function get_item( WP_REST_Request $request ) {
		...
		try {
			$wizard = \TVA\TTB\Apprentice_Wizard::get_data( $skin->ID );
		} catch ( Exception $e ) {
			$wizard = [];
		}
	...
	public static function fetch_templates( $request ) {
		...
		switch ( $type ) {
			...
			case 'module':
				...
				break;
			case 'lesson':
				$data = \TVA\TTB\Apprentice_Wizard::get_templates( $type, [], 'standard' );
				break;
			case 'video_lesson':
				$data = \TVA\TTB\Apprentice_Wizard::get_templates( $type, [], 'video' );
				break;
			case 'audio_lesson':
				$data = \TVA\TTB\Apprentice_Wizard::get_templates( $type, [], 'audio' );
				break;
			case 'sidebar':
