...
class TVA_Frontend_Controller extends TVA_REST_Controller {
	...
	public function mark_lesson( $request ) {
		...
		$shortcodes->set_course( $lesson_instance->get_course_v2()->get_id() );

		return new WP_REST_Response(
			[
				'lessons_list'     => do_shortcode( '[tva_lesson_list]' ),
				'progress_bar'     => do_shortcode( '[tva_progress_bar]' ),
				'message'          => TVA_Dynamic_Labels::get( 'course_progress' )['lesson_completed']['title'],
				'next_lesson_url'  => \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->set_data( $lesson_instance )->get_next_lesson_link(),
				'next_lesson_text' => \TVA\Architect\Dynamic_Actions\tcb_tva_dynamic_actions()->set_data( $lesson_instance )->get_next_lesson_text(),
			],
			200
		);
	}
