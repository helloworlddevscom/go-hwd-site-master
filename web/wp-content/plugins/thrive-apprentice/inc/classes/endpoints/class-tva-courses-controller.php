...
class TVA_Courses_Controller extends TVA_REST_Controller {
	...
	public function get_courses_v2( $request ) {
		...
		$limit = null !== ! $limit ? $limit : ( class_exists( 'TVA_Admin', false ) ? TVA_Admin::ITEMS_PER_PAGE : 10 );
		$limit = (int) $limit;
		...
		if ( $course ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( isset( $result->error_data['term_exists'] ) ) {
					...
					return new WP_Error( 'no-results', __( 'Something went wrong', 'thrive-apprentice' ) );
				}
			} else {
	...
	public function delete_course( $request ) {
		...
		if ( ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				if ( $post->post_type === \TVA\Drip\Campaign::POST_TYPE ) {
					( new \TVA\Drip\Campaign( $post ) )->delete();
				} else {
	...
	public function schedule_bulk_items( $items, $date ) {
		...
		foreach ( $items as $id ) {
			...
			if ( $post instanceof WP_Post && TVA_Const::LESSON_POST_TYPE === $post->post_type ) {
				wp_update_post( [
					'ID'            => $id,
					'post_date'     => $date,
	...
	public function get_items( $request ) {
		...
		$data     = array();
		$courses  = TVA_Course_V2::get_items( array(
			'status' => 'publish',
		) );
		...
		foreach ( $courses as $course ) {
			...
			if ( ! \TVA\Product_Migration::is_from_migration( $course->get_id() ) ) {
				continue;
			}
		...
		foreach ( $bundles as $item ) {
			...
			if ( ! \TVA\Product_Migration::is_from_migration( $item->number ) ) {
				continue;
			}
	...
	public function permalink_update( $request ) {
		...
		return rest_ensure_response( [ 'slug' => get_term( $course->term_id, TVA_Const::COURSE_TAXONOMY )->slug, 'preview_url' => $course->get_preview_url() ] );
	}
