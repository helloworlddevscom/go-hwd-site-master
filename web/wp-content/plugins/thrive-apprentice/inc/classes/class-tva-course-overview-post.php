...
final class TVA_Course_Overview_Post implements JsonSerializable {
	...
	private function _hooks() {
		...
		add_action(
			...
			9 //after tva_course() is initialized and before TAr reads the global post
		);
	...
	public function duplicate( $new_course ) {
		$overview_id         = $this->get_post()->ID;
		$new_course_overview = $new_course->overview_post->ensure_post();

		foreach ( get_post_meta( $overview_id ) as $meta_key => $post_meta_item ) {
			if ( isset( $post_meta_item[0] ) ) {
				if ( is_serialized( $post_meta_item[0] ) ) {
					update_post_meta( $new_course_overview->ID, $meta_key, thrive_safe_unserialize( $post_meta_item[0] ) );
				} else {
					update_post_meta( $new_course_overview->ID, $meta_key, $post_meta_item[0] );
				}
			}
		}
