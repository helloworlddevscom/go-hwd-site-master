 ...
 * @param \TVD\Content_Sets\Term_Rule $term_rule
 */
...
add_filter( 'tva_access_manager_get_content_sets_from_request', static function ( $matched = array(), $post_or_term = false ) {
	...
	if ( $should_query_for_course && in_array( get_post_type( $post_id ), [ \TVA_Const::LESSON_POST_TYPE, \TVA_Const::MODULE_POST_TYPE ] ) ) {
		...
		if ( is_array( $terms ) && isset( $terms[0] ) && $terms[0] instanceof WP_Term ) {
			 ...
			 */
			$matched = array_merge( $matched, \TVD\Content_Sets\Set::get_for_object( $terms[0], $terms[0]->term_id ) );
		}
		...
		if ( get_post_type( $post_id ) === \TVA_Const::LESSON_POST_TYPE ) {
			...
			if ( ! empty( $module_parent ) ) {
				 ...
				 */
				$matched = array_merge( $matched, \TVD\Content_Sets\Set::get_for_object( $module_parent->get_the_post(), $module_parent->ID ) );
			}
		}
	}
...
add_filter( 'tva_access_manager_get_login_redirect_url', static function ( $url ) {
	...
	if ( ! empty( \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_course() ) ) {
		$url = \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_course()->get_link();
	}
...
add_action( 'delete_user', static function () {
	...
	\TVA\Product::delete_count_users_with_access_cache( 0 );
} );
...
add_filter( 'tvd_global_data', static function ( $global_data ) {
	...
	$global_data[] = array(
		'name' => __( 'Courses page', TVA_Const::T ),
