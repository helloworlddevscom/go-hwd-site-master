...
function tva_check_ttb_version() {
	 ...
	 */
	if ( ! \TVA\TTB\Main::is_thrive_theme_active() ) {
		return true;
	}
...
function tva_hooks() {
	...
	$should_fire = ! is_admin() && ! \TVA\TTB\Apprentice_Wizard::is_frontend() && ! tve_dash_is_crawler();
	...
	if ( $should_fire ) {
		if ( tva_access_manager()->has_access() ) {
			 ...
			 * @api
			 */
			do_action( 'thrive_apprentice_restricted_course', \TVA\Architect\Visual_Builder\tcb_tva_visual_builder()->get_active_course(), tvd_get_current_user_details() );
		}
	}
}
...
function tva_send_hooks( $lesson_id, $state = 'start', $course_v2 = null ) {
	...
	if ( is_array( $learned_lessons ) ) {
		...
		foreach ( $learned_lessons as $course_id => $lessons_ids ) {
			...
			if ( ( $state === 'start' && $lesson_course_id === $course_id ) || ( $state === 'end' && count( $ids ) !== $course_v2->published_lessons_count ) ) {
				$course_found = true;
			}
		}
	}
	...
	if ( ! $lesson_found ) {
		...
		if ( $state === 'start' ) {
			...
			if ( $product instanceof \TVA\Product ) {
				...
				if ( count( $excluded_lesson_ids ) > 0 ) {

					/**
					 * This hook is triggered when the user finishes a lesson, and another one loads, in the same session. The hook will only be fired once per user, for each completed lesson.
					 * </br>
			...
			do_action( 'thrive_apprentice_course_progress', $course_v2, $user_details );
		}
	}
...
function tva_init() {
	...
	add_filter( 'thrive_ult_can_display_campaign', static function ( $can_display ) {
		if ( tva_is_inner_frame() || \TVA\TTB\Apprentice_Wizard::is_frontend() ) {
			$can_display = false;
		}
	...
	/* flush rewrite rules if needed ( this should only be needed when activating the plugin */
...
function tva_create_initial_rest_routes() {
	...
	foreach ( $endpoints as $e ) {
		...
		/* It might be possible that these classes are not loaded, in cases where an update is performed from Plesk (wp-cli) */
...
function tva_template_redirect() {
	...
	$should_redirect = $should_redirect && ! \TVA\TTB\Apprentice_Wizard::is_during_preview();
...
function tva_is_private_term() {
	...
	if ( tva_is_apprentice() ) {
		...
		$is_appr_content = \TVA\TTB\Check::course_item( $obj );
...
function tva_frontend_enqueue_scripts() {
	...
	if ( \TVA\TTB\Apprentice_Wizard::is_frontend() ) {
		tve_dash_enqueue_script( 'ttb-wizard-preview', THEME_ASSETS_URL . '/wizard.min.js', [ 'theme-frontend' ] );
		tve_dash_enqueue_style( 'ttb-wizard', THEME_ASSETS_URL . '/wizard.css' );
...
function tva_get_frontend_localization() {
	...
	if ( \TVA\TTB\Apprentice_Wizard::is_frontend() ) {
		$data['wizard'] = \TVA\TTB\tva_wizard()->localize_frontend();
	}
...
function tva_get_register_page( $login_form_bottom ) {
	...
	if ( tva_is_apprentice() ) {
		...
		if ( true === $register_page instanceof WP_Post ) {
			$login_form_bottom .= '<a class="tva-register-url" href="' . get_page_link( $register_page ) . '"> ' . __( 'Register a new account here', TVA_Const::T ) . '</a>';
		}
	}
...
function tva_is_module_started( $module ) {
	...
	$lessons = TVA_Manager::get_all_module_lessons( $module->get_the_post() );
...
function tva_get_start_course_url( $course ) {
	...
	if ( $course->published_lessons_count > 0 ) {
		$lessons      = TVA_Manager::get_all_lessons( $course instanceof WP_Term ? $course : $course->get_wp_term() );
...
function tva_mm_filter_access( $has_access ) {
	...
	if ( false === $has_access ) {
		...
		$sets = \TVD\Content_Sets\Set::get_for_object( $post, $post->ID );

		$product = \TVA\Product::get_from_set( $sets );

		if ( ! empty( $post ) && $product instanceof \TVA\Product ) {
...
function show_welcome_msg( $course ) {
	...
	$show_msg = $course->get_product() instanceof \TVA\Product && 'redirect' === tva_get_settings_manager()->factory( 'thankyou_page_type' )->get_value();
...
function tva_thrive_leads_skip_request( $skip ) {
	...
	if ( isset( $_REQUEST[ TVA_Const::TVA_FRAME_FLAG ] ) || \TVA\TTB\Apprentice_Wizard::is_frontend() ) {
		$skip = true;
	}
