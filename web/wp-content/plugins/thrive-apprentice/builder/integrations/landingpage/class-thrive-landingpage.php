...
class Thrive_Landingpage {
	...
	public static function actions() {
		...
		add_action( 'tcb_lp_import_sections', [ __CLASS__, 'import_lp_sections' ], 10, 3 );
	}
			...
			foreach ( $theme_sections as $type => $section ) {
				if ( isset( $section['id'] ) ) {
					...
					if ( empty( $section['id'] ) ) {
						...
						$export_data = json_encode( $theme_section->export() );
	 ...
	 * @return void
	 */
	public static function import_lp_sections( $page_id, $config, $import_instance ) {
		$sections = [];
