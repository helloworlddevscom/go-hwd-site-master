...
if ( thrive_automator_requirements() ) {
	...
	require_once TAP_PLUGIN_PATH . 'inc/classes/class-tap-update.php';
	Update::init();
}
...
function thrive_automator_requirements() {
	...
	if ( ! $ok ) {
		add_action( 'admin_notices', static function () use ( $error_message ) {
			echo sprintf( '<div class="notice notice-error error"><p>%s</p></div>', $error_message );
		} );
	}
