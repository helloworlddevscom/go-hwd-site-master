...
class Webhook extends Action {
	...
	public static function get_description() {
		return 'Send data in a webhook to an external service. Methods supported include POST, PUT and GET.';
	}
		...
		if ( empty( $webhook_url ) ) {
			...
			tap_logger( $this->get_automation_id() )->insert_log(
				[
					'send_webhook' => [
						$error_key => [
							'message'    => $success ? 'Send webhook successfully executed' : 'Send webhook failed',
							'label'      => 'Send webhook response',
							'is_success' => $success,
						],
					],
				],
