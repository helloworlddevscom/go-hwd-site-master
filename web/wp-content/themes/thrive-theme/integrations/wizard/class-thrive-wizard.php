	...
	public function get_hf_preview_content( $type, $id, $source = 'cloud' ) {
		...
		if ( $source === 'cloud' ) {
			...
			if ( is_wp_error( $template ) ) {
				return '<div style="text-align: center; color: red; padding: 20px">' . sprintf( __( 'There has been an error while generating the content: %s', 'thrive-theme' ), '<strong>' . esc_html( $template->get_error_message() ) . '</strong>' ) . '</div>';
			}
